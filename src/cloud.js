      ...
      dia.secs.forEach((sec) => sec.ids.forEach((exId) => {
        ...
        (log.sets || []).filter((x) => x && x.done).forEach((x, i) => {
          filas.push({
            ...
            tecnica_ok: x.f !== false, completada: true, nota: log.note || null,
          });
        });
      }));
    }
...
export async function pushTrote(trote, catalogo) {
  if (!CLOUD || !USER || !trote) return;
  try {
    const runs = (trote.runs || []).concat(catalogo && catalogo.SEED_RUNS ? catalogo.SEED_RUNS : []);
    const assign = Object.assign({}, catalogo && catalogo.SEED_ASSIGN, trote.assign || {});
    const vistos = {};
    const filas = runs.filter((r) => { if (vistos[r.id]) return false; vistos[r.id] = 1; return true; })
      .map((r) => ({ id: String(r.id), usuario: USER, fecha: r.date, slot: assign[r.id] || null,
        minutos: r.min || 0, km: r.km || 0, ritmo: r.pace || null,
        esfuerzo_relativo: r.re || null, en_cinta: !!r.indoor }));
    if (filas.length) await sb("corridas", { method: "POST", body: JSON.stringify(filas) });

    const protos = [];
    Object.entries(trote.weeks || {}).forEach(([semana, w]) => {
      Object.entries(w || {}).forEach(([slot, val]) => {
        if (!val || !val.p) return;
        protos.push({ id: USER + "|" + semana + "|" + slot, usuario: USER, semana, slot, receta: val.p, nota: val.p.nota || null });
      });
    });
    if (protos.length) await sb("protocolos", { method: "POST", body: JSON.stringify(protos) });
  } catch (e) {}
}
